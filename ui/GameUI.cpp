...
GameMode GameUI::askPostGame() {
    std::string response;
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

    TextUtils::slowPrint("\nWhat do you want to do?\n", 30);
    TextUtils::slowPrint("  [1] Play again in the same mode\n", 30);
    TextUtils::slowPrint("  [2] Change mode\n", 30);
    TextUtils::slowPrint("  [3] Change difficulty\n", 30);
    TextUtils::slowPrint("  [4] Quit\n", 30);
    TextUtils::slowPrint("Your choice: ", 30);

    std::getline(std::cin, response);

    if (response == "1") return GameMode::Replay;
    if (response == "2") return GameMode::ChangeMode;
    if (response == "3") return GameMode::ChangeDifficulty;
...
BotDifficulty GameUI::askBotDifficulty() {
  ...
  while (true) {
    ...
    switch (choice) {
      ...
      case 3:
        TextUtils::styledPrint("Bot difficulty set to Hard. Brace yourself!\n\n", FG_BRIGHT_RED);
...
void GameUI::printMessage(const std::string& msg) {
    std::cout << msg << "\n";
}
