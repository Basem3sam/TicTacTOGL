...
bool Game::play() {
    ...
    while (true) {
        ...
        GameMode choice = UI->askPostGame();

        if (choice == GameMode::Return) {
            break;

        } else if (choice == GameMode::Replay) {
            continue; // same mode, same difficulty

        } else if (choice == GameMode::ChangeDifficulty) {
            // only makes sense in SinglePlayer
            if (mode == GameMode::SinglePlayer) {
                BotDifficulty newDiff = UI->askBotDifficulty();
                // update the bot's difficulty
                if (auto bot = dynamic_cast<Bot*>(players[1].get())) {
                    bot->setDifficulty(newDiff);
                }
            } else {
                UI->printMessage("Difficulty only applies to Single Player mode.");
            }
            continue; // replay with new difficulty

        } else if (choice == GameMode::ChangeMode) {
            mode = setup();
            if (mode == GameMode::Return)
                break;
        }
